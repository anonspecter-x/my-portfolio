...
import { Layers, ArrowUpRight, Calendar, Sparkles, GitBranch, ExternalLink } from "lucide-react";

// 📌 এডমিন প্যানেলের নতুন ডেটা স্ট্রাকচার অনুযায়ী ইন্টারফেস আপডেট করা হলো
interface Project {
  _id: string;
  title: string;
  tagline?: string;
  ...
  backend?: string[];
  tools?: string[];
  tech?: string[]; // Fallback for older projects
}
...
export default function ProjectsClient({ projects = [] }: { projects: Project[] }) {
  ...
  return (
    <main className="relative min-h-screen bg-[#fafafa] dark:bg-[#030303] text-[#111] dark:text-[#f5f5f5] pt-32 pb-20 px-6 sm:px-8 md:px-12 max-w-[85rem] mx-auto overflow-hidden selection:bg-blue-500/30">
      ...
      >
        {/* 📌 Badge without shadow (Flat & Clean) */}
      ...
      {(projects?.length ?? 0) > 0 ? (
        ...
        >
          {projects?.map((project) => {
            // 📌 নতুন এডমিন প্যানেল থেকে frontend, backend এবং tools নিয়ে একটি লিষ্ট তৈরি করা হচ্ছে (সর্বোচ্চ ৪টি দেখানোর জন্য)
            const displayTech = [
              ...(project.frontend || []), 
              ...(project.backend || []), 
              ...(project.tools || []), 
            ...
            return (
              <motion.div key={project._id} variants={fadeUp} className="h-full">
                <div className="group flex flex-col bg-white dark:bg-[#0a0a0a] border border-gray-200 dark:border-gray-800 rounded-[2rem] overflow-hidden hover:border-blue-500/30 transition-all duration-500 shadow-sm hover:shadow-xl hover:-translate-y-1 h-full relative z-10">
                  
                  {/* 📸 Project Image Container */}
                  <Link href={`/projects/${project.slug}`} className="w-full aspect-[16/10] bg-gray-100 dark:bg-[#111] border-b border-gray-200 dark:border-gray-800 relative overflow-hidden block">
                    
                    {/* Floating Action Button (Arrow) */}
                    <div className="absolute top-6 right-6 bg-white dark:bg-[#111] w-10 h-10 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all translate-y-2 group-hover:translate-y-0 duration-300 border border-gray-200 dark:border-gray-800 z-20">
                  ...
                  <div className="p-8 md:p-10 flex flex-col flex-1">
                    ...
                    <div className="mb-6 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-3">
                        {/* 📌 GitHub & Live Links from Admin Data */}
                        {project.githubLink && (
                          <a href={project.githubLink} target="_blank" rel="noreferrer" className="text-gray-400 hover:text-black dark:hover:text-white transition-colors" title="Source Code">
                            <GitBranch className="w-4 h-4" />
                          </a>
                        )}
                        {project.liveLink && (
                          <a href={project.liveLink} target="_blank" rel="noreferrer" className="text-gray-400 hover:text-blue-500 transition-colors" title="Live Preview">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-6 border-t border-gray-100 dark:border-gray-800/60 mt-auto">
                      ...
                      {/* Tech Stack Chips (Mapped securely from unified array) */}
                      ...
                      {/* Year & Status */}
                      <span className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400 flex-shrink-0">
