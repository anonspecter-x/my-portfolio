...
export default function CertificatesClient({ certificates }: CertificatesClientProps) {
  ...
  return (
    <main className="relative min-h-screen bg-[#fafafa] dark:bg-[#030303] text-[#111] dark:text-[#f5f5f5] pt-32 pb-20 px-6 sm:px-8 md:px-12 max-w-[85rem] mx-auto overflow-hidden selection:bg-blue-500/30">
      ...
      {/* 🎨 Animated Background Elements (Standard Style) */}
      <motion.div 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {certificates.map((cert) => (
              <motion.div 
                key={cert._id} 
                variants={fadeUp} 
                className="group bg-white dark:bg-[#0a0a0a] border border-gray-200 dark:border-white/5 rounded-3xl p-5 md:p-6 shadow-sm hover:shadow-xl dark:hover:shadow-[0_8px_30px_-15px_rgba(255,255,255,0.05)] transition-all duration-500 flex flex-col hover:-translate-y-2 relative overflow-hidden"
              >
                
                {/* Top Section: Logo & Titles */}
                <div className="flex gap-4 items-start mb-6 relative z-10">
                  {cert.issuerLogo ? (
                    <div className="w-14 h-14 rounded-2xl bg-white dark:bg-[#111] border border-gray-100 dark:border-gray-800 shadow-sm flex items-center justify-center p-2 shrink-0 group-hover:scale-105 transition-transform duration-300">
                      <img src={cert.issuerLogo} alt={cert.issuerName} className="w-full h-full object-contain" />
                    </div>
                  ) : (
                    <div className="w-14 h-14 rounded-2xl bg-gray-100 dark:bg-[#111] border border-gray-200 dark:border-gray-800 flex items-center justify-center text-gray-400 shrink-0">
                      <Award className="w-6 h-6" />
                    </div>
                  )}
                  
                  <div className="flex-1 pt-1">
                    <h3 className="font-bold text-lg leading-snug text-black dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors line-clamp-2">
                      {cert.title}
                    </h3>
                ...
                {/* Middle Section: Image Showcase with Zoom Hint */}
                {cert.certificateImage && (
                  <div 
                    onClick={() => setSelectedImage(cert.certificateImage!)}
                    className="relative w-full aspect-[4/3] rounded-2xl overflow-hidden bg-gray-50 dark:bg-[#050505] border border-gray-100 dark:border-gray-800 cursor-zoom-in mb-6 group/img"
                  >
                    <img 
                    ...
                    <div className="absolute inset-0 bg-black/0 group-hover/img:bg-black/20 dark:group-hover/img:bg-black/40 transition-colors duration-300 flex items-center justify-center opacity-0 group-hover/img:opacity-100">
                ...
                {/* Bottom Section: Verification Link */}
                <div className="mt-auto pt-5 border-t border-gray-100 dark:border-white/5 flex items-center justify-between">
                  {cert.credentialUrl ? (
                    <a 
                      ...
                      target="_blank" 
                      rel="noreferrer" 
                      className="inline-flex items-center gap-1.5 text-sm font-bold text-black dark:text-white hover:text-blue-600 dark:hover:text-blue-400 transition-colors group/link"
                    >
