...
interface LoginPageProps {
  settings?: any; // 📌 Settings prop added to receive the dynamic logo
}
...
export default function LoginPage({ settings }: LoginPageProps) {
  ...
  // 📌 Dynamic Logo Logic (Imported from Header)
              ...
              <>
                {(settings?.siteLogoLight || settings?.siteLogo) && (
                  <img 
                    ...
                    alt={devFullName} 
                    className={`h-10 w-auto object-contain ${settings?.siteLogoDark ? 'block dark:hidden' : ''}`} 
                  />
                )}
                {settings?.siteLogoDark && (
                  <img 
                    ...
                    alt={devFullName} 
                    className={`h-10 w-auto object-contain ${(settings?.siteLogoLight || settings?.siteLogo) ? 'hidden dark:block' : ''}`} 
                  />
                )}
              </>
            ) : (
              <>
                <span className="w-10 h-10 rounded-full bg-black dark:bg-white flex items-center justify-center text-white dark:text-black shadow-sm">
        ...
        <form action={formAction} className="flex flex-col gap-5">
          ...
          {errorMessage && (
            <motion.div 
              ...
              className="flex items-start gap-2.5 bg-red-50 dark:bg-red-950/30 text-red-600 dark:text-red-400 p-3.5 rounded-xl text-sm border border-red-100 dark:border-red-900/50 shadow-sm"
            >
