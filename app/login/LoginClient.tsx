...
import { ArrowRight, Lock, Mail, AlertCircle, Loader2, ShieldCheck, ArrowLeft, Code2 } from "lucide-react";
import Link from "next/link";
import { useActionState, useEffect, useState } from "react";
import { authenticate } from "./actions"; // আপনার সঠিক পাথ দিন
  ...
  return (
    <main className="relative min-h-screen flex items-center justify-center bg-[#fafafa] dark:bg-[#030303] px-4 sm:px-6 overflow-hidden selection:bg-blue-500/30">
      ...
      <motion.div 
        ...
        className="w-full max-w-[420px] bg-white/80 dark:bg-[#0a0a0a]/80 backdrop-blur-2xl border border-gray-200 dark:border-white/5 rounded-[2.5rem] p-8 sm:p-10 shadow-2xl dark:shadow-[0_8px_40px_-15px_rgba(0,0,0,0.8)] relative z-10"
      >
        
        {/* 📌 Dynamic Logo Placement (Based on Header logic) */}
              ...
              <>
                {(settings?.siteLogoLight || settings?.siteLogo) && (
                  <img 
                    ...
                    alt={devFullName} 
                    className={`h-10 w-auto object-contain transition-transform duration-500 group-hover:scale-105 ${settings?.siteLogoDark ? 'block dark:hidden' : ''}`} 
                  />
                )}
                {settings?.siteLogoDark && (
                  <img 
                    ...
                    alt={devFullName} 
                    className={`h-10 w-auto object-contain transition-transform duration-500 group-hover:scale-105 ${(settings?.siteLogoLight || settings?.siteLogo) ? 'hidden dark:block' : ''}`} 
                  />
                )}
              </>
            ) : (
              <>
                <span className="w-10 h-10 rounded-full bg-black dark:bg-white flex items-center justify-center text-white dark:text-black shadow-sm transition-transform duration-500 group-hover:scale-105">
                  <Code2 className="w-5 h-5" />
                </span>
                <span className="font-extrabold text-2xl tracking-tighter text-black dark:text-white">
                  {logoText}
                </span>
              </>
            )}
        ...
        <div className="mb-8 text-center">
          <div className="inline-flex items-center w-fit gap-1.5 px-3 py-1.5 rounded-full bg-blue-50 dark:bg-blue-500/10 border border-blue-100 dark:border-blue-500/20 text-[10px] font-black uppercase tracking-widest mb-4 text-blue-600 dark:text-blue-400">
            <ShieldCheck className="w-3.5 h-3.5" /> Secure Authentication
          </div>
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-black dark:text-white mb-2">
            Admin Portal
          </h1>
        ...
        <form action={formAction} className="flex flex-col gap-5">
          ...
          <AnimatePresence>
            {errorMessage && (
              <motion.div 
                ...
                className="flex items-start gap-2.5 bg-red-50 dark:bg-red-950/30 text-red-600 dark:text-red-400 p-3.5 rounded-xl text-sm border border-red-100 dark:border-red-900/50 shadow-sm overflow-hidden"
              >
          ...
          <div className="flex flex-col gap-2">
            <label className="text-[10px] font-bold tracking-widest uppercase text-gray-500 dark:text-gray-400 ml-1">
              Email Address
            </label>
        ...
        <div className="mt-8 pt-6 border-t border-gray-100 dark:border-white/5 text-center">
          <Link 
            href="/" 
            className="inline-flex items-center justify-center gap-1.5 text-xs font-bold text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white transition-colors group"
          >
